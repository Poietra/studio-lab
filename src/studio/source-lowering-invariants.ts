...
export function scaleTransformViolation(operations: readonly CanonicalEditOperation[]) {
  ...
  for (const operation of operations) {
    ...
    const transform = transforms.find((candidate) => (
      (related.get(candidate.sourceEntityId) ?? new Set(operationEntityIds(candidate)))
        .has(operation.entityId)
    ));
...
function isTransitionInternalOperation(
  ...
  return operation.effect === "reveal"
...
export function sceneBoundaryViolation(operations: readonly CanonicalEditOperation[]) {
  const boundaries = operations.filter((operation): operation is Extract<CanonicalEditOperation, {
    kind: "InsertSceneBoundary";
  }> => operation.kind === "InsertSceneBoundary");
  const overlayEntityIds = new Set(operations.flatMap((operation) => (
  ...
  for (const boundary of boundaries) {
    for (const operation of operations) {
      ...
      if (
        operation.interval.start >= boundary.at - EPSILON
        || operation.interval.end > boundary.at + EPSILON
      ) {
