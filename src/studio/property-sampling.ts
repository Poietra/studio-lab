 ...
 */
export function normalizePositionSamples(
  ...
  const baseIndex = samples.findIndex((sample) => (
    sample.kind === "exact"
    && sample.operationId === undefined
    && sample.interval.start === firstStart
  ));
  const ordered = samples
    ...
    .sort((left, right) => {
      ...
      return sameStartPriority(left.sample, left.index, baseIndex)
        - sameStartPriority(right.sample, right.index, baseIndex)
  ...
  for (const { sample } of ordered) {
    ...
    const control = sample.control
      ? {
          x: (from.x + value.x) / 2
            + sample.control.x - (sample.from.x + sample.value.x) / 2,
          y: (from.y + value.y) / 2
            + sample.control.y - (sample.from.y + sample.value.y) / 2,
        }
      : undefined;
    normalized.push({ ...sample, control, from, value });
  }
  return normalized;
}
