...
export function replaceScaleKeyframeProgram(
  ...
  if (input.keyframes[0] && input.keyframes[0].time <= fadeEnd + KEYFRAME_EPSILON) {
    throw new TypeError("The first scale keyframe must be after the object's initial fade.");
  }
  ...
  const hasCompetingScale = input.baseProgram.operations.some(
    (operation) =>
      operation.kind === "AnimateProperty" && operation.key === "scale" && operation.timelineTrack !== true,
  );
  if (input.keyframes.length > 0 && hasCompetingScale) {
    throw new TypeError("Remove the object's existing scale edit before adding a scale keyframe track.");
  }
