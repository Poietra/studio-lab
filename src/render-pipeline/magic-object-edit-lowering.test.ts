...
describe("Magic Edit scale/delete source lowering", () => {
  it("exports one canonical scale-then-delete sequence without dropping either effect", () => {
    ...
    if (!imported) return;
    const entity = Object.values(imported.runtimeSceneState.objectGraph.entities).find(
      (candidate) => candidate.sourceIdentity.kind === "known" && candidate.sourceIdentity.value === "equation",
    );
    ...
    expect(lowered.insertedCode.indexOf("equation.animate.scale(1.5)")).toBeLessThan(
      lowered.insertedCode.indexOf("FadeOut(equation)"),
    );

    const roundTrip = importManimScene(lowered.source, "scene.py", "MagicObjectEdits");
    expect(roundTrip?.runtimeSceneState.propertyChannels[`${entity.id}/scale`]?.samples.at(-1)).toMatchObject({
      value: 1.5,
    });
    expect(roundTrip?.runtimeSceneState.objectGraph.entities[entity.id]?.lifetime.at(-1)?.end).toBeCloseTo(6.4);
  });
