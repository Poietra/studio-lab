    ...
    async ({ mode, mp4Base64, normalizedRegion, sampleTimes }) => {
      ...
      try {
        ...
        for (const sampleTime of sampleTimes) {
          ...
          stats.push({
            averageBlue: bright === 0 ? Number.NaN : totalBlue / bright,
...
test("paints imported SVG and a closed Pen path with WGSL through reload and MP4 export", async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Add Angle material keyframe for CubicBezier" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    let penMaterialEnd = page.getByRole("button", { name: /Angle material parameter keyframe 2 at/u });
    await penMaterialEnd.click();
    await page.getByLabel("Angle material parameter keyframe value").fill("-3.1");
    await page.getByRole("button", { name: "Replace program" }).click();
    penMaterialEnd = page.getByRole("button", { name: /Angle material parameter keyframe 2 at/u });
    await penMaterialEnd.click();
    await page.getByLabel("Angle material parameter keyframe time").fill("1.9");
    await page.getByRole("button", { name: "Replace program" }).click();
    let penMaterialStart = page.getByRole("button", { name: /Angle material parameter keyframe 1 at/u });
    await penMaterialStart.click();
    await page.getByRole("combobox", { name: "Angle material parameter segment easing" }).selectOption("linear");
    await page.getByRole("button", { name: "Replace program" }).click();
    ...
    penDrawClip = page.getByRole("button", { name: "Edit CubicBezier Draw entrance" });
    await expect(penDrawClip).toHaveAttribute("title", "Draw 0.40–2.10s · smooth");
    penMaterialStart = page.getByRole("button", { name: /Angle material parameter keyframe 1 at/u });
    penMaterialEnd = page.getByRole("button", { name: /Angle material parameter keyframe 2 at/u });
...
test("draws a Studio Line through scrub, retime, history, reload, and MP4 export", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole("combobox", { name: "Assigned fragment material" })).not.toHaveValue("");
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    const waveSpeed = page.getByRole("slider", { name: "Speed material parameter" });
    await waveSpeed.fill("0");
    await expect(waveSpeed).toHaveValue("0");
    const linePlayhead = page.getByRole("slider", { name: "Scene playhead" });
    const lineMaterialParameter = page.getByRole("combobox", { name: "Material parameter for Line" });
    await lineMaterialParameter.selectOption("Bands");
    await linePlayhead.fill("2");
    await page.getByRole("button", { name: "Add Bands material keyframe for Line" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    await linePlayhead.fill("2.4");
    await page.getByRole("button", { name: "Add Bands material keyframe for Line" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    const bandsStart = page.getByRole("button", { name: /Bands material parameter keyframe 1 at/u });
    const bandsEnd = page.getByRole("button", { name: /Bands material parameter keyframe 2 at/u });
    await bandsEnd.click();
    await page.getByLabel("Bands material parameter keyframe value").fill("1");
    await page.getByRole("button", { name: "Replace program" }).click();
    await bandsStart.click();
    await page.getByRole("combobox", { name: "Bands material parameter segment easing" }).selectOption("linear");
    await page.getByRole("button", { name: "Replace program" }).click();

    await lineMaterialParameter.selectOption("Speed");
    await linePlayhead.fill("2.6");
    await page.getByRole("button", { name: "Add Speed material keyframe for Line" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    const speedStart = page.getByRole("button", { name: /Speed material parameter keyframe 1 at/u });
    await expect(speedStart).toBeVisible();
    await expect(bandsStart).toHaveCount(0);
    await expect(bandsEnd).toHaveCount(0);
    await page.getByRole("button", { name: "Undo" }).click();
    await expect(speedStart).toHaveCount(0);
    await lineMaterialParameter.selectOption("Bands");
    await expect(bandsStart).toBeVisible();
    await expect(bandsEnd).toBeVisible();
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    await page.getByRole("button", { name: "Redo" }).click();
    await lineMaterialParameter.selectOption("Bands");
    await expect(bandsStart).toBeVisible();
    ...
    await linePlayhead.fill("3");
    await page.getByRole("button", { name: "Add Speed material keyframe for Line" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    const speedEnd = page.getByRole("button", { name: /Speed material parameter keyframe 2 at/u });
    await speedEnd.click();
    await page.getByLabel("Speed material parameter keyframe value").fill("1.1");
    await page.getByRole("button", { name: "Replace program" }).click();
    await speedStart.click();
    await page.getByRole("combobox", { name: "Speed material parameter segment easing" }).selectOption("ease-in");
    await page.getByRole("button", { name: "Replace program" }).click();
    await lineMaterialParameter.selectOption("Bands");
    await bandsEnd.click();
    await expect(page.getByLabel("Bands material parameter keyframe value")).toHaveValue("1");
    await lineMaterialParameter.selectOption("Speed");
    await expect(speedEnd).toBeVisible();
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    ...
    await expect(page.getByRole("button", { name: "Use solid stroke" })).toBeVisible();
    await expect(page.getByRole("combobox", { name: "Assigned fragment material" })).not.toHaveValue("");
    await expect(lineMaterialParameter).toHaveValue("Speed");
    await expect(speedStart).toBeVisible();
    await expect(speedEnd).toBeVisible();
    await expect(bandsStart).toHaveCount(0);
    await expect(bandsEnd).toHaveCount(0);
    await speedStart.click();
    await expect(page.getByLabel("Speed material parameter keyframe time")).toHaveValue("2.2");
    await expect(page.getByRole("combobox", { name: "Speed material parameter segment easing" })).toHaveValue(
      "ease-in",
    );
    await speedEnd.click();
    await expect(page.getByLabel("Speed material parameter keyframe time")).toHaveValue("2.6");
    await expect(page.getByLabel("Speed material parameter keyframe value")).toHaveValue("1.1");
    await lineMaterialParameter.selectOption("Bands");
    await expect(speedStart).toHaveCount(0);
    await expect(speedEnd).toHaveCount(0);
    await expect(bandsStart).toBeVisible();
    await expect(bandsEnd).toBeVisible();
    await bandsStart.click();
    await expect(page.getByLabel("Bands material parameter keyframe time")).toHaveValue("1.6");
    await expect(page.getByRole("combobox", { name: "Bands material parameter segment easing" })).toHaveValue("linear");
    await bandsEnd.click();
    await expect(page.getByLabel("Bands material parameter keyframe time")).toHaveValue("2");
    await expect(page.getByLabel("Bands material parameter keyframe value")).toHaveValue("1");
    await lineMaterialParameter.selectOption("Speed");
    await expect(speedStart).toBeVisible();
    await expect(speedEnd).toBeVisible();
    ...
    const mp4 = await exportLocalMp4(page);
    const exportedStrokePixels = await decodedBrightPixelCounts(page, mp4, [0.02, 0.75, 2.1], "green-dominant");
    ...
    const materialStats = await decodedPixelStats(page, mp4, [1.65, 1.95, 2.05, 2.15, 2.25, 2.55]);
    expect(materialStats.every(({ count }) => count > 100)).toBe(true);
    const bandsChange = materialStats[1];
    const stableControl = materialStats[3];
    const speedChange = materialStats[5];
    for (const sample of [bandsChange, stableControl, speedChange]) {
      expect(sample?.commonPixelCountFromPrevious ?? 0).toBeGreaterThan(50);
    }
    const averageCommonColorDifference = (sample: (typeof materialStats)[number] | undefined) =>
      (sample?.commonColorDifferenceFromPrevious ?? 0) / (sample?.commonPixelCountFromPrevious || 1);
    expect(averageCommonColorDifference(bandsChange)).toBeGreaterThan(averageCommonColorDifference(stableControl) + 2);
    expect(averageCommonColorDifference(speedChange)).toBeGreaterThan(averageCommonColorDifference(stableControl) + 2);
  } finally {
    if (projectId) await cleanupFixtureWorkspace(page.request, { projectId });
  }
});
...
test("writes a Studio MathTex through scrub, history, reload, and MP4 export", async ({ page }) => {
  ...
  try {
    ...
    await page.getByLabel("Cool material color").fill("#ff0000");
    await page.getByLabel("Warm material color").fill("#000000");
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    const scenePlayhead = page.getByRole("slider", { name: "Scene playhead" });
    const equationMaterialParameter = page.getByRole("combobox", { name: "Material parameter for E = mc^2" });
    await equationMaterialParameter.selectOption("Angle");
    ...
    await page.getByRole("button", { name: "Add Angle material keyframe for E = mc^2" }).click();
    await page.getByRole("button", { name: "Replace program" }).click();
    let equationMaterialEnd = page.getByRole("button", { name: /Angle material parameter keyframe 2 at/u });
    await equationMaterialEnd.click();
    await page.getByLabel("Angle material parameter keyframe value").fill("-3.1");
    await page.getByRole("button", { name: "Replace program" }).click();
    let equationMaterialStart = page.getByRole("button", { name: /Angle material parameter keyframe 1 at/u });
    ...
    await expect(equationMaterial).not.toHaveValue("");
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    equationMaterialStart = page.getByRole("button", { name: /Angle material parameter keyframe 1 at/u });
    equationMaterialEnd = page.getByRole("button", { name: /Angle material parameter keyframe 2 at/u });
    await expect(equationMaterialStart).toBeVisible();
    await expect(equationMaterialEnd).toBeVisible();
    ...
    equationCoolEnd = page.getByRole("button", { name: /Cool material parameter keyframe 2 at/u });
    await expect(equationCoolStart).toBeVisible();
    await expect(equationCoolEnd).toBeVisible();
    await equationCoolStart.click();
    await expect(page.getByLabel("Cool material parameter keyframe value")).toHaveValue("#ff0000");
    await equationCoolEnd.click();
    await expect(page.getByLabel("Cool material parameter keyframe value")).toHaveValue("#0000ff");
    const equationCoolStartTime = await propertyKeyframeTime(equationCoolStart);
    const equationCoolEndTime = await propertyKeyframeTime(equationCoolEnd);
    expect(equationCoolStartTime).toBeCloseTo(equationMaterialStartTime);
    expect(equationCoolEndTime).toBeCloseTo(equationMaterialEndTime);
    await scenePlayhead.fill((equationMaterialStartTime + 0.05).toFixed(2));
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    await scenePlayhead.fill(((equationCoolStartTime + equationCoolEndTime) / 2).toFixed(2));
    await expect(canvas).toHaveAttribute("data-preview-renderer", "presented");
    ...
    const exportedMathTexStats = await decodedPixelStats(
      ...
      [
        0.02,
        0.75,
        equationCoolStartTime + 0.05,
        (equationCoolStartTime + equationCoolEndTime) / 2,
        equationCoolEndTime + 0.05,
      ],
      "bright",
      equationRegion,
    );
    ...
    const colorStart = exportedMathTexStats[2];
    const colorMiddle = exportedMathTexStats[3];
    const changedMaterial = exportedMathTexStats[4];
    expect(colorStart?.averageRed ?? 0).toBeGreaterThan((colorStart?.averageBlue ?? 0) + 30);
    expect(colorMiddle?.averageRed ?? 0).toBeGreaterThan(30);
    expect(colorMiddle?.averageBlue ?? 0).toBeGreaterThan(30);
    expect(changedMaterial?.averageBlue ?? 0).toBeGreaterThan((changedMaterial?.averageRed ?? 0) + 30);
