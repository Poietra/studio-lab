...
await Promise.all([
  ...
  mkdir(resolve(layout.appRoot, "electron"), { recursive: true }).then(() =>
    cp(resolve(repositoryRoot, "electron", "preload.cjs"), resolve(layout.appRoot, "electron", "preload.cjs")),
  ),
]);
await writeFile(
  resolve(layout.appRoot, "package.json"),
  `${JSON.stringify(
    {
