        ...
        D: Deserializer<'de>,
    {
        match deserialize_js_safe_u64(deserializer)? {
            ...
            version => Err(de::Error::custom(format!(
                "unsupported fast-manim snapshot profile version {version}; expected 1, 2, 3, 4, 5, 6, 7, 8, or 9"
            ))),
        }
    }
}
...
mod integer_wire_tests {
    ...
    fn snapshot_profile_versions_match_javascript_number_semantics() {
        ...
        assert_eq!(
            serde_json::from_str::<SnapshotProfileVersionV1>("5.0").unwrap(),
            SnapshotProfileVersionV1::V5
        );
        assert_eq!(
            serde_json::from_str::<SnapshotProfileVersionV1>("6.0").unwrap(),
            SnapshotProfileVersionV1::V6
        );
        assert_eq!(
            serde_json::from_str::<SnapshotProfileVersionV1>("7.0").unwrap(),
            SnapshotProfileVersionV1::V7
        );
        assert_eq!(
            serde_json::from_str::<SnapshotProfileVersionV1>("8.0").unwrap(),
            SnapshotProfileVersionV1::V8
        );
        assert_eq!(
            serde_json::from_str::<SnapshotProfileVersionV1>("9.0").unwrap(),
            SnapshotProfileVersionV1::V9
        );
        assert!(serde_json::from_str::<SnapshotProfileVersionV1>("10.0").is_err());
    }
}
