...
/// Encoder queue depth above which a push awaits `dequeue` events. Frames are
/// never dropped; the session only waits.
pub(crate) const MAX_ENCODE_QUEUE_DEPTH_V1: u32 = 8;

/// Forced key-frame cadence in microseconds (2 seconds).
pub(crate) const KEY_FRAME_INTERVAL_MICROSECONDS_V1: f64 = 2_000_000.0;
...
/// High profile level 4.0 first, constrained baseline level 3.1 fallback.
pub(crate) const H264_CODEC_LADDER_V1: [&str; 2] = ["avc1.640028", "avc1.42E01F"];
...
pub(crate) const MAX_TOTAL_ENCODED_BYTES_V1: u64 = 256 * 1024 * 1024;
...
impl ExportEncoderSessionConfigV1 {
    /// Duration of one frame in microseconds at the declared frame rate.
    pub(crate) fn frame_duration_microseconds(&self) -> f64 {
        1_000_000.0 / f64::from(self.frames_per_second)
    }
...
pub(crate) fn key_frame_required_v1(
    ...
    match last_key_frame_timestamp_microseconds {
        None => true,
        Some(last) => timestamp_microseconds - last >= KEY_FRAME_INTERVAL_MICROSECONDS_V1,
    }
}
...
mod tests {
    ...
    fn valid_session_request_parses_into_the_exact_config() {
        ...
        assert!((config.frame_duration_microseconds() - 33_333.333_333).abs() < 1e-3);
    }
    ...
    fn key_frame_cadence_starts_keyed_and_repeats_every_two_seconds() {
        assert!(key_frame_required_v1(None, 0.0));
        assert!(!key_frame_required_v1(Some(0.0), 1_999_999.0));
        assert!(key_frame_required_v1(Some(0.0), 2_000_000.0));
        assert!(key_frame_required_v1(Some(2_000_000.0), 4_000_000.0));
        assert!(!key_frame_required_v1(Some(2_000_000.0), 3_999_999.9));
    }
    ...
    fn responses_serialize_with_schema_version_and_bounded_size() {
        for result in [
            ...
            ExportEncoderResultV1::Chunk {
                byte_length: 1_024,
                duration_microseconds: Some(33_333.0),
                index: 3,
                key_frame: false,
                timestamp_microseconds: 100_000.0,
            },
    ...
    fn ladder_and_bounds_are_the_declared_closed_contract() {
        assert_eq!(H264_CODEC_LADDER_V1, ["avc1.640028", "avc1.42E01F"]);
        assert_eq!(MAX_ENCODE_QUEUE_DEPTH_V1, 8);
        assert!((KEY_FRAME_INTERVAL_MICROSECONDS_V1 - 2_000_000.0).abs() < f64::EPSILON);
    }
}
