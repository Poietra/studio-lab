...
mod tests {
    ...
    fn text_boundary_compiles_ascii_and_japanese_blocks_without_changing_mathtex_contract() {
        ...
        assert_eq!(
            decode(&japanese)["result"]["fragments"]
                .as_array()
                .unwrap()
                .len(),
            14
        ...
        let mono = compile_text_outline_json_v1(
            br#"{"layout":{"alignment":"left","fontFamily":"mono","fontWeight":"bold","lineHeight":1.2},"schema":"poietra.text-outline-request","version":1,"text":"Hello AV"}"#,
        );
        assert_eq!(decode(&mono)["result"]["kind"], "compiled");
        assert_ne!(
            decode(&mono)["result"]["path"],
            decode(&bold)["result"]["path"]
        );

        let mixed_mono = compile_text_outline_json_v1(
            "{\"layout\":{\"alignment\":\"left\",\"fontFamily\":\"mono\",\"fontWeight\":\"bold\",\"lineHeight\":1.2},\"schema\":\"poietra.text-outline-request\",\"version\":1,\"text\":\"Mono日本語\"}"
                .as_bytes(),
        );
        assert_eq!(decode(&mixed_mono)["result"]["kind"], "compiled");
    ...
    #[test]
    fn maximum_glyph_count_text_response_stays_compiled_under_the_boundary_limit() {
        let text = format!("{}\n{}", "i".repeat(127), "i".repeat(128));
        ...
        .unwrap();
        let response = compile_text_outline_json_v1(&request);
        assert!(response.len() <= MAX_MATHTEX_OUTLINE_RESPONSE_JSON_BYTES_V1);
        let decoded = decode(&response);
        assert_eq!(decoded["result"]["kind"], "compiled");
        assert_eq!(
            decoded["result"]["fragments"].as_array().unwrap().len(),
            255
        );
    }
}
