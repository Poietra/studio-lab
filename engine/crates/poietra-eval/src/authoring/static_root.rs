...
impl EngineSessionV1 {
    ...
    /// Replays static position history and repeated rigid rotations or uniform selection resizes
    /// for independent imported roots. Mixed resize/rotation histories are applied in Program
    /// order to the same isolated candidate used by the single-family histories.
    ///
    ...
    /// operation stays in the projection for browser history, source lowering, Undo, and Redo.
    #[allow(
        clippy::float_cmp,
        clippy::too_many_lines,
    ...
    fn apply_static_root_transform_history(
        ...
        let rotation_program_indexes = programs
            ...
            .collect::<Vec<_>>();
        let scale_program_indexes = programs
            .iter()
            .enumerate()
            .filter_map(|(index, program)| {
                program
            ...
            .collect::<Vec<_>>();
        for index in &rotation_program_indexes {
        ...
        for index in &scale_program_indexes {
            ...
            if !(2..=8).contains(&positions)
                || positions != scales
                || program.operations.len() != positions + scales
            {
                return Err(ApplyStaticRootTransformEditError::Unsupported);
            }
        }
        ...
        let provenance_id = format!("studio-static-transform:{next_revision}");
        if scene
            .provenance
            .iter()
            .any(|record| record.id == provenance_id)
        {
            return Err(TransformSceneEntityError::ProvenanceConflict(provenance_id).into());
        }
        let mut candidate = SceneIrBundleV1 {
            assets: self.assets().clone(),
            scene: scene.clone(),
        ...
        for (program_index, program) in programs.iter().enumerate() {
            ...
            let mut program_scale_studio_entity_ids = BTreeSet::new();
            let mut program_scale_runtime_entity_ids = BTreeSet::new();
            let mut program_scale_factor = None;
            for scheduled_id in &program.schedule_order {
                ...
                match &operation.kind {
                    ...
                    StaticRootTransformOperationKind::Rotation {
                        ...
                        to: Some(to),
                    } if rotation_program_indexes.contains(&program_index)
                    ...
                    StaticRootTransformOperationKind::UniformScale {
                        ...
                        && program_position_studio_entity_ids.contains(studio_entity_id)
                        && program_position_runtime_entity_ids.contains(&runtime_entity_id)
                        && program_scale_studio_entity_ids.insert(studio_entity_id.to_owned())
                        && program_scale_runtime_entity_ids.insert(runtime_entity_id.clone())
                        && program_scale_factor.is_none_or(|factor: f64| {
                            close_transform_baseline_value(factor, *relative_factor)
                        }) =>
                    {
                        program_scale_factor.get_or_insert(*relative_factor);
                        let bounds = scene_entity_local_bounds(candidate_entity)
                            .ok_or(ApplyStaticRootTransformEditError::Unsupported)?;
                        let pivot = scene_entity_world_center(candidate_entity, &bounds);
                        apply_world_axis_scale(
            ...
            if scale_program_indexes.contains(&program_index) {
                ...
                for (entity_id, from) in &program_position_from {
                    let to = program_position_to
                        .get(entity_id)
                        .ok_or(ApplyStaticRootTransformEditError::Unsupported)?;
                    if !close_transform_baseline_value(
                        to.x - first_to.x,
                        (from.x - first_from.x) * factor,
                    ) || !close_transform_baseline_value(
                        to.y - first_to.y,
                        (from.y - first_from.y) * factor,
                    ) {
                        return Err(ApplyStaticRootTransformEditError::Unsupported);
                    }
                }
        ...
        candidate.scene.provenance.push(ProvenanceRecordV1 {
            evidence: operation_ids
                ...
                .chain(std::iter::once(
                    if !rotation_program_indexes.is_empty() && !scale_program_indexes.is_empty() {
                        "Studio imported-root composed selection transform"
                    } else if !rotation_program_indexes.is_empty() {
    ...
    pub fn apply_static_root_transform_edit(
        ...
        if command
            ...
            .sum::<usize>()
            >= 2
            && command.programs.iter().all(|program| {
            ...
            && (command
                .programs
                .iter()
                .flat_map(|program| program.operations.iter())
                .filter(|operation| {
                    matches!(
                        operation.kind,
                        StaticRootTransformOperationKind::UniformScale { .. }
...
mod tests {
    ...
    fn static_root_group_position_command() -> ApplyStaticRootTransformEditCommand {
        let mut command = static_root_position_command();
        let mut earlier_move = command.programs[0].operations[0].clone();
        earlier_move.entity_id = Some("source:earlier".to_owned());
        earlier_move.id = "move-earlier".to_owned();
        earlier_move.kind = StaticRootTransformOperationKind::Position {
            position: Some(PointV1 { x: 240.0, y: 180.0 }),
        };
        command.programs[0].operations.push(earlier_move);
        command.programs[0]
            .schedule_order
            .push("move-earlier".to_owned());
        command
            .source_runtime_bindings
        ...
        command
            ...
            .push(StaticRootTransformStudioEntity {
                ...
                object_graph_key: "source:earlier".to_owned(),
                position: Some(PointV1 { x: 280.0, y: 180.0 }),
                provisional: false,
                scale: Some(1.0),
                source_identity: Some("earlier".to_owned()),
                transaction_id: None,
            });
        command
    }
    ...
    fn append_group_rotation_program(command: &mut ApplyStaticRootTransformEditCommand) {
        let mut next = static_root_group_rotation_command().programs.remove(0);
        next.transaction_id = "rotate-imported-selection-again".to_owned();
        for operation in &mut next.operations {
            let entity_id = operation.entity_id.as_deref().unwrap();
            operation.id = format!("rotate-again:{entity_id}:{}", operation.id);
            if matches!(
                operation.kind,
                StaticRootTransformOperationKind::Position { .. }
            ) {
                operation.kind = StaticRootTransformOperationKind::Position {
                    position: Some(if entity_id == "source:circle" {
                        PointV1 { x: 280.0, y: 180.0 }
                    } else {
                        PointV1 { x: 360.0, y: 180.0 }
                    }),
                };
            }
        }
    ...
    fn static_root_group_resize_command() -> ApplyStaticRootTransformEditCommand {
        let mut command = static_root_group_position_command();
        for operation in &mut command.programs[0].operations {
            let position = match operation.entity_id.as_deref() {
                Some("source:circle") => PointV1 { x: 380.0, y: 180.0 },
                Some("source:earlier") => PointV1 { x: 260.0, y: 180.0 },
                _ => continue,
            };
            operation.kind = StaticRootTransformOperationKind::Position {
                position: Some(position),
        ...
        let mut scales = command.programs[0]
            .operations
            .iter()
            .enumerate()
            .map(|(index, operation)| {
                let mut scale = operation.clone();
                scale.id = format!("scale-{index}");
        ...
        command.programs[0]
            .schedule_order
            .extend(scales.iter().map(|operation| operation.id.clone()));
        command.programs[0].operations.append(&mut scales);
        command
    }
    ...
    fn append_resize_after_rotation_program(command: &mut ApplyStaticRootTransformEditCommand) {
        let mut next = static_root_group_resize_command().programs.remove(0);
        next.transaction_id = "resize-after-rotation".to_owned();
        for operation in &mut next.operations {
            let entity_id = operation.entity_id.as_deref().unwrap();
            operation.id = format!("after-rotation:{}", operation.id);
            if matches!(
                operation.kind,
                StaticRootTransformOperationKind::Position { .. }
            ) {
                operation.kind = StaticRootTransformOperationKind::Position {
                    position: Some(if entity_id == "source:circle" {
                        PointV1 { x: 320.0, y: 120.0 }
                    } else {
                        PointV1 { x: 320.0, y: 240.0 }
                    }),
                };
            }
        }
    ...
    fn append_rotation_after_resize_program(command: &mut ApplyStaticRootTransformEditCommand) {
        let mut next = static_root_group_rotation_command().programs.remove(0);
        next.transaction_id = "rotation-after-resize".to_owned();
        for operation in &mut next.operations {
            let entity_id = operation.entity_id.as_deref().unwrap();
            operation.id = format!("after-resize:{}", operation.id);
            if matches!(
                operation.kind,
                StaticRootTransformOperationKind::Position { .. }
            ) {
    ...
    fn append_group_position_program(
        ...
        let mut next = static_root_group_position_command().programs.remove(0);
        next.transaction_id = transaction_id.to_owned();
        for operation in &mut next.operations {
            let entity_id = operation.entity_id.as_deref().unwrap();
            operation.id = format!("{transaction_id}:{entity_id}");
            operation.kind = StaticRootTransformOperationKind::Position {
                position: Some(if entity_id == "source:circle" {
                    later.clone()
                } else {
    ...
    #[test]
    fn applies_one_atomic_position_program_to_independent_imported_roots() {
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();

        let result = session
            .apply_static_root_transform_edit(static_root_group_position_command())
            .unwrap();
        ...
        assert_eq!(
            result
                .static_root_projection
                .as_ref()
                .map(|projection| projection.mutations.len()),
            Some(2)
        );
        let later = result
            .bundle
        ...
        let earlier = result
            ...
            .unwrap();
        assert!((later.transform.tx - 1.0).abs() < f64::EPSILON);
    ...
    #[test]
    fn replays_repeated_imported_group_rotation_history() {
        let mut command = static_root_group_rotation_command();
        append_group_rotation_program(&mut command);
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        ...
        for (entity_id, expected_center_x) in [("later", -1.0), ("earlier", 1.0)] {
            let entity = result
                .bundle
                .scene
                .entities
            ...
            assert!((entity.transform.m11 + 1.0).abs() < 1e-12);
            assert!((entity.transform.m22 + 1.0).abs() < 1e-12);
            assert!(entity.transform.m12.abs() < 1e-12);
            assert!(entity.transform.m21.abs() < 1e-12);
        }
    }

    #[test]
    fn uniformly_resizes_independent_imported_roots_as_one_selection() {
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();

        let result = session
            .apply_static_root_transform_edit(static_root_group_resize_command())
            .unwrap();
        let projection = result.static_root_projection.as_ref().unwrap();
        ...
        for (entity_id, expected_center_x) in [("later", 1.5), ("earlier", -1.5)] {
            let entity = result
                .bundle
                .scene
                .entities
            ...
            assert!((center.x - expected_center_x).abs() < 1e-12);
            assert!(center.y.abs() < 1e-12);
            assert!((entity.transform.m11 - 1.5).abs() < 1e-12);
            assert!((entity.transform.m22 - 1.5).abs() < 1e-12);
        }
    }
    ...
    fn rejects_non_uniform_imported_group_resize_atomically() {
        ...
        last_scale.kind = StaticRootTransformOperationKind::UniformScale {
            control_present: false,
            from: Some(1.0),
        ...
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        assert!(matches!(
            session.apply_static_root_transform_edit(mismatched_factor),
            Err(ApplyStaticRootTransformEditError::Unsupported)
        ));
        assert_eq!(session.scene().source.revision_hash(), BASE_REVISION);

        let mut mismatched_position = static_root_group_resize_command();
        mismatched_position.programs[0].operations[0].kind =
            StaticRootTransformOperationKind::Position {
                position: Some(PointV1 { x: 390.0, y: 180.0 }),
            };
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        assert!(matches!(
    ...
    #[test]
    fn composes_imported_group_rotation_and_resize_in_program_order() {
        let mut rotation_then_resize = static_root_group_rotation_command();
        append_resize_after_rotation_program(&mut rotation_then_resize);
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        let rotation_then_resize = session
            .apply_static_root_transform_edit(rotation_then_resize)
            .unwrap();

        let mut resize_then_rotation = static_root_group_resize_command();
        append_rotation_after_resize_program(&mut resize_then_rotation);
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        let resize_then_rotation = session
            .apply_static_root_transform_edit(resize_then_rotation)
            .unwrap();

        for result in [&rotation_then_resize, &resize_then_rotation] {
            assert_eq!(
                result
                    .static_root_projection
                    .as_ref()
            ...
            assert_eq!(result.bundle.scene.source.revision_hash(), NEXT_REVISION);
            for (entity_id, expected_center_y) in [("later", 1.5), ("earlier", -1.5)] {
                let entity = result
                    .bundle
                    .scene
                    .entities
                ...
                assert!(entity.transform.m11.abs() < 1e-12);
                assert!(entity.transform.m22.abs() < 1e-12);
                assert!((entity.transform.m12.abs() - 1.5).abs() < 1e-12);
                assert!((entity.transform.m21.abs() - 1.5).abs() < 1e-12);
            }
        }
    }

    #[test]
    fn rejects_malformed_imported_group_rotation_resize_composition_atomically() {
        let mut mismatched_position = static_root_group_rotation_command();
        append_resize_after_rotation_program(&mut mismatched_position);
        mismatched_position.programs[1].operations[0].kind =
            StaticRootTransformOperationKind::Position {
                position: Some(PointV1 { x: 321.0, y: 120.0 }),
            };
        ...
        let mut stale_scale_baseline = static_root_group_rotation_command();
        append_resize_after_rotation_program(&mut stale_scale_baseline);
        stale_scale_baseline.programs[1].operations[2].kind =
            StaticRootTransformOperationKind::UniformScale {
                control_present: false,
                from: Some(1.25),
                relative_factor: Some(1.2),
                to: Some(1.5),
            };
        let bundle = static_imported_bundle();
        let mut session = EngineSessionV1::new(bundle.clone()).unwrap();
        assert!(matches!(
            session.apply_static_root_transform_edit(stale_scale_baseline),
            Err(ApplyStaticRootTransformEditError::Unsupported)
        ));
        assert_eq!(session.scene(), &bundle.scene);
    }
    ...
    #[test]
    fn replays_complete_position_history_after_a_prior_supported_program() {
        let mut command = static_root_group_position_command();
        let mut initial = static_root_position_command().programs.remove(0);
        initial.operations[0].id = "initial-position:source:circle".to_owned();
        initial.operations[0].kind = StaticRootTransformOperationKind::Position {
            position: Some(PointV1 { x: 380.0, y: 180.0 }),
        };
        initial.schedule_order[0] = initial.operations[0].id.clone();
        initial.transaction_id = "initial-position".to_owned();
        command.programs.insert(0, initial);
        append_group_position_program(
            &mut command,
            "second-selection-move",
            &PointV1 { x: 440.0, y: 180.0 },
            &PointV1 { x: 320.0, y: 180.0 },
        );

        let mut preview = EngineSessionV1::new(static_imported_bundle()).unwrap();
        let preview = preview.apply_static_root_transform_edit(command).unwrap();
        assert_eq!(
            preview
                .static_root_projection
                .as_ref()
                ...
                .collect::<Vec<_>>(),
            vec![
                "initial-position:source:circle",
                "move-circle",
                "move-earlier",
                "second-selection-move:source:circle",
                "second-selection-move:source:earlier",
            ]
        );
        let later = preview
            .bundle
        ...
        let earlier = preview
            ...
            .find(|entity| entity.id == "earlier")
            .unwrap();
        assert!((later.transform.tx - 2.0).abs() < f64::EPSILON);
        assert!((earlier.transform.tx - 1.0).abs() < f64::EPSILON);
    }

    #[test]
    fn replays_group_position_history_that_returns_to_the_imported_baseline() {
        let mut command = static_root_group_position_command();
        append_group_position_program(
            &mut command,
            "return-selection-to-origin",
            &PointV1 { x: 360.0, y: 180.0 },
            &PointV1 { x: 280.0, y: 180.0 },
        );
        let mut session = EngineSessionV1::new(static_imported_bundle()).unwrap();
        ...
        for entity_id in ["later", "earlier"] {
            let entity = result
                ...
                .find(|entity| entity.id == entity_id)
                .unwrap();
            assert!(entity.transform.tx.abs() < f64::EPSILON);
            assert!(entity.transform.ty.abs() < f64::EPSILON);
        }
    }

    #[test]
    fn rejects_group_position_for_nested_or_mixed_transform_targets_atomically() {
        let mut nested_bundle = static_imported_bundle();
        nested_bundle
            .scene
            .entities
        ...
        let mut nested = EngineSessionV1::new(nested_bundle).unwrap();
        assert!(matches!(
            nested.apply_static_root_transform_edit(static_root_group_position_command()),
            Err(ApplyStaticRootTransformEditError::Unsupported)
        ));
        assert_eq!(nested.scene().source.revision_hash(), BASE_REVISION);

        let mut mixed_command = static_root_group_position_command();
        mixed_command.programs[0].operations[1].kind =
            StaticRootTransformOperationKind::UniformScale {
                control_present: false,
                from: Some(1.0),
                relative_factor: Some(2.0),
        ...
        let mut mixed = EngineSessionV1::new(static_imported_bundle()).unwrap();
        assert!(matches!(
            mixed.apply_static_root_transform_edit(mixed_command),
            Err(ApplyStaticRootTransformEditError::Unsupported)
        ));
        assert_eq!(mixed.scene().source.revision_hash(), BASE_REVISION);
    }
