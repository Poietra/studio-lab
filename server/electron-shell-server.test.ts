...
async function waitForRender(origin: string, id: string) {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const response = await fetch(`${origin}/api/manim/renders/${id}`);
...
describe("Electron shell HTTP adapter", () => {
  it("serves a locked-down renderer and reuses CRUD, render, and export contracts", async () => {
    ...
    const documentResponse = await fetch(shell.origin);
    ...
    expect((await fetch(`${shell.origin}/../package.json`)).status).toBe(404);

    const projectsResponse = await fetch(`${shell.origin}/api/manim/projects`);
    ...
    const workspace = await (await fetch(`${shell.origin}/api/manim/projects/shell-project/workspace`)).json() as {
    ...
    const renderResponse = await fetch(`${shell.origin}/api/manim/projects/shell-project/renders`, {
    ...
    await expect(waitForRender(shell.origin, started.id)).resolves.toMatchObject({ status: "ready" });
    const video = await fetch(`${shell.origin}/api/manim/renders/${started.id}/video`);
    expect(video.headers.get("content-type")).toBe("video/mp4");
    expect((await video.arrayBuffer()).byteLength).toBeGreaterThan(0);

    const exported = await fetch(`${shell.origin}/api/manim/projects/shell-project/export`, {
      body: JSON.stringify(request(scene.sourceHash, entityId!)),
      headers: { "content-type": "application/json" },
      method: "POST",
    });
    ...
    await fetch(`${shell.origin}/api/manim/renders/${started.id}/discard`, { method: "POST" });

    const managed = await (await fetch(`${shell.origin}/api/manim/projects`, {
      body: JSON.stringify({ kind: "managed", name: "Managed" }),
      headers: { "content-type": "application/json" },
      method: "POST",
    })).json() as { project: { id: string } };
    const renamed = await fetch(`${shell.origin}/api/manim/projects/${managed.project.id}`, {
      body: JSON.stringify({ name: "Renamed" }),
      headers: { "content-type": "application/json" },
      method: "PATCH",
    });
    expect(await renamed.json()).toMatchObject({ project: { name: "Renamed" } });
    expect((await fetch(`${shell.origin}/api/manim/projects/${managed.project.id}`, { method: "DELETE" })).status).toBe(200);
