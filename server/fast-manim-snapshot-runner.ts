...
import {
  ...
  type RuntimeTraceEditPreflight,
  type RuntimeTraceMoveEditPreflight,
  type RuntimeTraceOpacityEditPreflight,
  type RuntimeTraceResizeEditPreflight,
  type RuntimeTraceRotationEditPreflight,
...
import {
  ...
  MAX_FAST_MANIM_RUNTIME_TRACE_SOURCE_BYTES_V3,
...
// Preview-cache accounting and admission control live in
// ./fast-manim-snapshot-preview-cache; the subprocess supervision state
// machine lives in ./fast-manim-snapshot-producer-process. Both are
// re-exported below so the public surface of this module is unchanged.
export {
...
const DEFAULT_CACHE_RETENTION_MS = 30 * 60_000;
...
function parseServerOwnedFastManimRunView(value: unknown): FastManimSnapshotRunViewV1 {
  ...
  return {
    ...
    // reference; the preview cache retains a separate frozen copy.
...
export type FastManimRuntimeTraceCandidateRunRequestV1 = Omit<FastManimRuntimeTraceRunRequestV1, "sourceHash"> &
  Readonly<{
    moveEdit?: RuntimeTraceMoveEditPreflight;
    opacityEdit?: RuntimeTraceOpacityEditPreflight;
    rotationEdit?: RuntimeTraceRotationEditPreflight;
    resizeEdit?: RuntimeTraceResizeEditPreflight;
  }>;
...
const RUNTIME_TRACE_FAILURE_MESSAGES: Readonly<Record<FastManimRuntimeTraceRunFailureCodeV1, string>> = {
  ...
  "unsupported-profile": "Runtime Trace could not analyze the requested Scene.",
};
...
export class FastManimSnapshotRunner {
  ...
  constructor(
    ...
    const maxCachedSnapshots = options.maxCachedSnapshots ?? DEFAULT_MAX_CACHED_SNAPSHOTS;
    if (!Number.isSafeInteger(maxCachedSnapshots) || maxCachedSnapshots <= 0) {
      throw new TypeError("Maximum cached snapshots must be a positive integer.");
    }
    const maxCachedBytes = options.maxCachedBytes ?? DEFAULT_MAX_CACHED_BYTES;
    if (!Number.isSafeInteger(maxCachedBytes) || maxCachedBytes <= 0) {
      throw new TypeError("Maximum cached snapshot bytes must be a positive integer.");
    }
    const cacheRetentionMs = options.cacheRetentionMs ?? DEFAULT_CACHE_RETENTION_MS;
    if (!Number.isSafeInteger(cacheRetentionMs) || cacheRetentionMs <= 0) {
      throw new TypeError("Cached snapshot retention must be a positive integer of milliseconds.");
    }
    ...
    this.maxCachedSnapshots = maxCachedSnapshots;
    ...
    this.previewCache = options.previewCache ?? processSnapshotCache;
    this.cacheRetentionMs = cacheRetentionMs;
  ...
  get busy() {
    // Cache lookups revalidate asynchronously, so they hold the runner
  ...
  /** Verify one sandbox result without assigning process-local preview-cache state. */
  ...
  /** Execute a verified Runtime Trace preview without preview-cache state. */
  ...
  private async runRuntimeTraceLocked(
    ...
    let runtimeConfigHash = ZERO_SHA256;
    ...
    try {
      runtimeConfigHash = digestFastManimRuntimeTraceConfigV3(createFastManimRuntimeTraceConfigV3(this.frame));
    } catch {
    ...
    try {
      ...
      producerRequest = createFastManimRuntimeTraceProducerRequestV3(
        request,
        before.source,
        {
    ...
    let bundle;
    let producerEvidence: FastManimRuntimeTraceProducerEvidenceV2;
    let roots: FastManimRuntimeTraceVerifiedRootsV2;
    try {
    ...
    const currentRuntimeConfigHash = digestFastManimRuntimeTraceConfigV3(
      createFastManimRuntimeTraceConfigV3(this.frame),
    );
    ...
    if (source.kind !== "imported-manim-runtime-trace" || source.traceVersion !== 3) {
      return failed("result-rejected");
    }
  ...
  /** Executes one generic edit preflight without preview-cache state. */
  async runRuntimeTraceCandidateUnpublished(
    ...
    const { moveEdit, opacityEdit, resizeEdit, rotationEdit, ...runtimeRequestValue } = requestValue;
    if ([moveEdit, opacityEdit, resizeEdit, rotationEdit].filter(Boolean).length !== 1) {
      throw new TypeError("A generic Runtime Trace candidate carries exactly one edit authority.");
    }
    const preflight =
      moveEdit?.kind === "runtime-trace-move-edit"
        ? moveEdit
        : opacityEdit?.kind === "runtime-trace-opacity-edit"
          ? opacityEdit
          : resizeEdit?.kind === "runtime-trace-resize-edit"
            ? resizeEdit
            : rotationEdit?.kind === "runtime-trace-rotation-edit"
              ? rotationEdit
              : undefined;
    ...
    if (
      ...
      Buffer.byteLength(sourceText, "utf8") > MAX_FAST_MANIM_RUNTIME_TRACE_SOURCE_BYTES_V3
    ) {
      throw new RangeError(
        `Runtime Trace candidate source accepts at most ${MAX_FAST_MANIM_RUNTIME_TRACE_SOURCE_BYTES_V3} UTF-8 bytes.`,
      );
    }
    ...
    if (this.closing) throw new HttpError("The Manim render pipeline is shutting down.", 503);
    if (request.projectId !== this.projectId) throw new HttpError("Configured Manim project not found.", 404);
    if (candidateSourceHash === preflight.baseSourceHash) {
    ...
    const pending = this.runRuntimeTraceEditCandidateLocked(sourceText, request, preflight, signal);
    this.activeRuns.add(pending);
    pending.catch(() => undefined);
    try {
  ...
  private async runRuntimeTraceEditCandidateLocked(
    ...
    const plan = (() => {
      try {
        return preflight.kind === "runtime-trace-move-edit"
          ? deriveRuntimeTraceMoveSourceEditPlan(
              sourceText,
              request.sceneName,
              request.sourcePath,
              preflight.baseBinding.name,
              preflight.sourceAnchor,
            )
        ...
        throwIfHalted();
        throw new HttpError(
          "The generic Runtime Trace candidate source edit could not be re-derived. Reimport and try again.",
          409,
        );
      }
    })();
    const planMatchesPreflight =
      "expectedWorldCenter" in plan
        ...
        : "expectedOpacity" in plan
          ? preflight.kind === "runtime-trace-opacity-edit" && plan.expectedOpacity === preflight.expectedOpacity
    ...
    if (
      ...
      plan.sourceAnchor !== preflight.sourceAnchor ||
      ...
      throw new HttpError(
        "The generic Runtime Trace candidate base or source binding is stale. Reimport and try again.",
        409,
      );
    }

    const baseAnalysis = studioSourceAnalysisProviderV1.analyze({
      expectedSourceHash: before.hash,
      sceneName: request.sceneName,
      sourcePath: request.sourcePath,
      sourceText: before.source,
    });
    const candidateAnalysis = studioSourceAnalysisProviderV1.analyze({
    ...
    const baseProducerRequest = createFastManimRuntimeTraceProducerRequestV3(
      ...
      before.source,
      {
        constructStartLine: baseAnalysis.scene.construct.span.startLine,
        definitionOrdinal: baseAnalysis.scene.ordinal,
      },
      this.frame,
      baseBindings,
    );
    const candidateProducerRequest = createFastManimRuntimeTraceProducerRequestV3(
      request,
      sourceText,
      {
        constructStartLine: candidateAnalysis.scene.construct.span.startLine,
        definitionOrdinal: candidateAnalysis.scene.ordinal,
      },
      this.frame,
      candidateBindings,
    );
    ...
    try {
      ...
      throwIfHalted();
      throw new HttpError("The generic Runtime Trace candidate sandbox is unavailable.", 503);
    }
    throwIfHalted();
    ...
    const produceTrace = async (producerRequest: typeof baseProducerRequest) => {
      ...
      throwIfHalted();
      if (result.kind !== "ok") {
        throw new HttpError("The generic Runtime Trace candidate was rejected by its sandbox.", 409);
      }
      return parseFastManimRuntimeTraceProducerJsonV3(
        result.resultBytes,
        producerRequest,
        trustedFastManimRuntimeTraceProducerV3(),
      );
    ...
    const verified =
      preflight.kind === "runtime-trace-move-edit"
        ? verifyFastManimRuntimeTraceMoveEditCandidateV3({
            ...
            sourceAnchor: preflight.sourceAnchor,
          })
        : preflight.kind === "runtime-trace-opacity-edit"
          ? verifyFastManimRuntimeTraceOpacityEditCandidateV3({
              ...candidatePair,
    ...
    try {
      ...
      throwIfHalted();
      throw new HttpError("The generic Runtime Trace candidate base changed during verification.", 409);
    }
    throwIfHalted();
    ...
    return Object.freeze({
      ...
      traceDigest: digestFastManimRuntimeTraceV3(verified),
    });
  }
  ...
  private async runRequest(
    ...
    const pending = this.runLocked(request, cacheLocally, signal);
    this.activeRuns.add(pending);
    pending.catch(() => undefined);
    try {
      return await pending;
  ...
  private async runLocked(
    ...
    if (encodedBytes > this.maxCachedBytes || (cacheLocally && !this.previewCache.fitsSingleEntry(encodedBytes))) {
    ...
    const key = sceneKey(request.sourcePath, request.sceneName);
    const { cachedAt, cacheSequence } = this.cacheVerified(key, expected, sealed, sourceRuntimeIdentity, encodedBytes);
    this.logger.info("snapshot.cached", { cacheSequence, requestId: request.requestId });
    return {
      ...verified,
      // Wire-frozen compatibility fields (ADR 0005, `publication`/`revision`
      // dispositions): the run view keeps the `publishedAt`/`revision` names,
      // but locally they carry the preview cache's insertion time and
      // cache-global sequence, not a durable Publication generation.
  ...
  private localOwnerId() {
    this.ownerId ??= this.previewCache.registerOwner();
    return this.ownerId;
  }

  private evictExpired(ownerId: number, now: number) {
    for (const [key, entry] of this.previewCache.entriesOf(ownerId)) {
      if (now - entry.cachedAtEpochMs > this.cacheRetentionMs) this.previewCache.delete(ownerId, key);
    }
  }

  private cacheVerified(
    key: string,
    ...
    // Cache sequences come from the shared cache's single monotonic sequence,
    // so neither eviction, TTL expiry, nor another tenant's entries can ever
    // make a freshly recached Scene appear older than a stale cached copy.
    const cacheSequence = this.previewCache.nextCacheSequence();
    const cachedAt = new Date(now).toISOString();
    const evicted = this.previewCache.insert(
      ownerId,
      key,
      { cachedAt, cachedAtEpochMs: now, cacheSequence, encodedBytes, expected, result, sourceRuntimeIdentity },
      { maxBytes: this.maxCachedBytes, maxEntries: this.maxCachedSnapshots },
    );
    if (evicted > 0) this.logger.info("snapshot.evicted", { evicted });
    return { cachedAt, cacheSequence };
  }
  ...
  private async produce(
    ...
    signal?: AbortSignal,
    runtimeTraceResultMaxBytes = MAX_FAST_MANIM_RUNTIME_TRACE_JSON_BYTES_V3,
  ...
  private async lookupLocked(query: FastManimSnapshotQueryV1): Promise<FastManimSnapshotRunViewV1> {
    ...
    // longer current the lookup retries against the freshly cached entry instead
  ...
  private async lookupCurrentEntry(query: FastManimSnapshotQueryV1): Promise<FastManimSnapshotRunViewV1 | null> {
    ...
    const entry = this.previewCache.get(ownerId, key);
    ...
    const entryIsCurrent = () => this.previewCache.peek(ownerId, key) === entry;
    const deleteIfCurrent = () => {
      if (entryIsCurrent()) this.previewCache.delete(ownerId, key);
    };
    if (Date.now() - entry.cachedAtEpochMs > this.cacheRetentionMs) {
    ...
    const staleView = (): FastManimSnapshotRunViewV1 | null => {
      ...
      // recache superseded it, the fresh entry must survive and the lookup
      // retries instead of reporting the dead cache sequence.
      if (!entryIsCurrent()) return null;
      this.previewCache.delete(ownerId, key);
      return fastManimSnapshotRunViewV1Schema.parse({
        ...base,
        fallback: FAST_MANIM_SNAPSHOT_FALLBACK_V1,
        // Wire-frozen `revision` field (ADR 0005): carries the cache sequence.
        revision: entry.cacheSequence,
        status: "stale",
      });
    };
    ...
    throwIfClosing();
    // Never serve an entry that a concurrent recache already superseded.
    if (!entryIsCurrent()) return null;
    return parseServerOwnedFastManimRunView({
      ...base,
      // Wire-frozen compatibility fields (ADR 0005): `publishedAt`/`revision`
      // carry the preview cache's insertion time and cache sequence here.
      publishedAt: entry.cachedAt,
      revision: entry.cacheSequence,
  ...
  private async closeLocked() {
    ...
    // Wait for every in-flight run and cache lookup to settle so nothing
    // caches or serves after close, then return this runner's preview-cache
    // accounting to the shared budget.
    ...
    try {
      ...
      if (this.ownerId !== null) this.previewCache.releaseOwner(this.ownerId);
    }
  }
}
