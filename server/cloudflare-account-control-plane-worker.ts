...
function organizationMutationRateLimitKey(request: Request, operation: "create" | "member") {
  const sessionTokenHash = accountSessionTokenHashV1(request.headers.get("cookie"));
  if (sessionTokenHash === null) return null;
  const fingerprint = createHash("sha256")
    ...
    .update(sessionTokenHash)
    .digest("base64url");
  return `account-organization:${operation}:${fingerprint}`;
}
...
export function createCloudflareAccountControlPlaneWorkerV1(
  ...
  return Object.freeze({
    async fetch(request: Request, environment: CloudflareAccountControlPlaneEnvironmentV1) {
      ...
      const isMembership = pathname === ACCOUNT_MEMBERS_ROUTE_V1 || pathname.startsWith(`${ACCOUNT_MEMBERS_ROUTE_V1}/`);
      const isMembershipMutation = pathname.startsWith(`${ACCOUNT_MEMBERS_ROUTE_V1}/`);
      ...
      if (
        ...
        try {
          ...
          if (isOrganization || isMembershipMutation) {
            const key = organizationMutationRateLimitKey(request, isOrganization ? "create" : "member");
          ...
          if (isOrganization || isMembershipMutation) return organizationMutationResponse(503);
