...
This experiment supports seven leaf operations: a new 2D translation motion, transforming one selected MathTex object into new mathematical content, transforming selected MathTex into explanatory Text, creating one explanatory Text object beside one selected object with FadeIn, creating a new MathTex entity, focusing the camera while emphasizing selected objects, and creating one Scene-level geometric transition to the next Scene. First decompose the whole request into all independently requested supported effects. Return a standalone leaf for one effect. Return edit-program for two or three motion/MathTex-transform/explanation effects; do not force the user to choose one effect merely because the sentence contains multiple verbs. create-camera-focus, create-equation, create-text-transform, and create-scene-transition are standalone in this version; if one is mixed with an incompatible effect, return one focused clarification naming that boundary. Each operation kind may occur at most once in one program. If the request contains only rotation, arbitrary scale, opacity, deletion, or another unsupported effect, return one focused clarification.
