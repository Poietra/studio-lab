CREATE TABLE poietra_schema_migrations (
...
CREATE INDEX workspace_projects_active_order
  ON workspace_projects (tenant_id, created_at, project_id)
  WHERE deleted_at IS NULL;

CREATE TABLE source_blob_objects (
  tenant_id text NOT NULL,
...
CREATE TABLE workspace_source_heads (
  tenant_id text NOT NULL,
  ...
  FOREIGN KEY (tenant_id, project_id)
    REFERENCES workspace_projects (tenant_id, project_id) ON DELETE RESTRICT,
  FOREIGN KEY (tenant_id, digest)
    REFERENCES source_blob_objects (tenant_id, digest) ON DELETE RESTRICT
);

CREATE TABLE workspace_project_references (
  tenant_id text NOT NULL,
  project_id text NOT NULL,
...
CREATE TABLE source_blob_deletions (
