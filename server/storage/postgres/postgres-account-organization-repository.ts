  ...
  async ready(signal?: AbortSignal) {
    try {
      const result = await this.#connection.query<{ checksum: string; version: number }>(
        "SELECT version, checksum FROM public.poietra_schema_migrations WHERE version = 34",
        [],
        signal,
      );
      ...
      return (
        result.rowCount === 1 &&
        result.rows[0]?.version === 34 &&
        result.rows[0]?.checksum === ACCOUNT_ORGANIZATION_LIFECYCLE_MIGRATION_V34_CHECKSUM
      );
    } catch {
      throwIfAborted(signal);
      return false;
