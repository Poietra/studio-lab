...
of the retained four-sample color target, plus the logical RGBA8 bytes of retained
image textures; it is not a claim about physical VRAM allocation.
...
| renderer capability coverage | partial | non-convex closed cubic fills, multiple subpaths, holes, self-intersections, nonzero/even-odd rules, general cubic strokes with v1 caps/joins/miter limits, ordered fill-then-stroke composition, transforms/camera/animation, verified PNG images, and four-sample coverage antialiasing work; the shared stroke fixture also samples nonzero trim, morph, and motion. Open fill paths and clipping remain truthful fallbacks. |
...
| retained-boundary memory budget | pending a fresh six-workload run after four-sample adoption | the prior largest post-fence high-water was 11,534,336 bytes for 1,000 animated cubics. The retained 1,920 x 1,080 RGBA8 four-sample target adds an exact 33,177,600-byte component, but the complete high-water must be remeasured because the WASM binary also changed. Transient image allocation and browser/driver RSS remain explicitly excluded. |
