...
verified `SceneIrBundleV1`. Canvas ABI v6 correlates the installed base revision
...
| atomic Scene replacement | met | Canvas Worker ABI v6 installs or replaces one complete verified snapshot; Rust validates and indexes the candidate before the swap, stale or invalid updates preserve the installed Scene, and imported Manim snapshots use the same boundary. |
